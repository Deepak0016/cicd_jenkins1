pipeline {
  ...
  stages {
    ...
    stage('Static Code Analysis') {
      environment {
        SONAR_URL = "http://35.192.112.40:9000/"
      }
      steps {
    ...
    stage('Build and Push Docker Image') {
      environment {
        DOCKER_IMAGE = "deepaksahootech/ultimate-cicd:${BUILD_NUMBER}"
            ...
            withCredentials([string(credentialsId: 'github', variable: 'GITHUB_TOKEN')]) {
                sh '''
                   # Navigate to the existing repo or clone it
                    ...
                    git config user.email "shehanon1@gmail.com"
                    git config user.name "Shehan"
